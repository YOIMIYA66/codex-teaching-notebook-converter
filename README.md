...
高信息密度不是单独目标，而是视觉优先压缩的实现手段。工作流会先确定每张图替代哪些章节和学习点，再生成高密度图片，最后压缩周围的 Markdown：

- 被图片覆盖的章节，叙述性文字目标减少约 40%–65%。
- 每张正文图通常合并至少 2 个碎片化章节和 3 个学习点。
- 每张图预计替代约 200 个以上字符的文字解释。
- 删除重复流程描述、重复标签列表和重复指标结论。
- 保留 1–2 句结论、来源链接、无障碍摘要、命令、参数、路径、表格和风险提示。
- 不把 notebook 变成纯图片；精确检索和复制仍由结构化文本承担。

完整规则见 `references/visual-first-compression.md`。
